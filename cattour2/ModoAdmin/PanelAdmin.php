...
<?php if (isset($_GET['status'])): ?>
    ...
    <?php elseif ($_GET['status'] === 'error_validacion'): ?>
        <div style="position: fixed; top: 20px; right: 20px; background: #dc3545; color: white; padding: 15px 25px; border-radius: 8px; font-weight: bold; z-index: 9999;">
            ❌ Error: Datos incompletos o inválidos (Días/Precio).
        </div>
    <?php elseif ($_GET['status'] === 'error_fecha'): ?>
        <div style="position: fixed; top: 20px; right: 20px; background: #dc3545; color: white; padding: 15px 25px; border-radius: 8px; font-weight: bold; z-index: 9999;">
            ❌ Error: La fecha de salida debe ser igual o mayor al año actual.
        </div>
    <?php elseif ($_GET['status'] === 'error_imagen'): ?>
        <div style="position: fixed; top: 20px; right: 20px; background: #dc3545; color: white; padding: 15px 25px; border-radius: 8px; font-weight: bold; z-index: 9999;">
            ❌ Error: No se logró procesar el viaje. El archivo subido es diferente al solicitado (.jpg, .jpeg, .png, .webp).
        </div>
    <?php endif; ?>
<?php endif; ?>
...
<div class="sidebar">
    ...
    <a href="../auth/logoutEmploy.php" style="background: var(--rojo-danger); margin-top: 50px;">Cerrar Sesión</a>
</div>
