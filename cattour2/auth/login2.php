...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Ya no necesitamos real_escape_string porque usaremos sentencias preparadas
    $usuario = $_POST['usuario'];
    $password = $_POST['password'];

    // 1. Preparamos la consulta SQL (Usamos ? en lugar de la variable)
    $stmt = $conn->prepare("SELECT idUsuario, nombre, password FROM UsuarioCliente WHERE usuario = ?");
    
    // 2. Vinculamos el parámetro (La "s" indica que es un string)
...
<head>
    ...
    <style>
        ...
        /* Botón Acceso Personal (Blanco para resaltar) */
        .btn-employ-corner {
            position: absolute;
            top: 25px;
            left: 25px;
...
<a href="LoginEmploy.php" class="btn-employ-corner">
    <span>🔑</span> Acceso Personal
</a>
